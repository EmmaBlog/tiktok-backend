...
export default async function handler(req, res) {
  try {
    ...
    const author = {
      ...
      avatar:
        item.author?.avatar_larger?.url_list?.slice(-1)[0] || "",
    };
    ...
    const musicUrl =
      item.music?.play_url?.url_list?.slice(-1)[0] || null;
    ...
    const sizeMB = await getSize(videoData.hd);

    if (download === "true") {
      const videoRes = await fetch(videoData.hd);
    ...
    return res.json({
      ...
      stats,
      download: {
        no_watermark: videoData.nowm,
        watermark: videoData.wm,
        hd: videoData.hd,
      },
      music: {
        url: musicUrl,
...
// Strongest video extractor
function extractVideo(item) {
  const wm =
    item.video?.download_addr?.url_list?.slice(-1)[0] || null;

  const nowm =
    item.video?.play_addr?.url_list?.slice(-1)[0]
      ?.replace("playwm", "play") || null;
